...
def run_TCS_model(x0, time, params, setpoint, input, output = 'solve_ivp'): 
    ...
    # # Initial conditions
    # # Comment it out when simulating the pi or pid-control regulation
    # # You only need to uncomment when you are fitting the parameters and IC. 
    # x0 = np.zeros(10)
    # x0[0] = p['S_0'] # S
    # x0[1] = p['Sp_0']# Sp
    # x0[2] = p['R_0'] # R
    ...
    # x0[5] = p['mRNA_0'] # mRNA
    # x0[6] = 0 # Ctic
    # x0[7] = p['P_0'] # Unfolded Protein 
    # x0[8] = p['Pm_0'] # Folded Protein 
    # x0[9] = p['C_0'] # Initial Cell count

    # # sol = scint.odeint(dydt, x0, time, args = (params, input))
    sol = scint.solve_ivp(dydt, [time[0], time[-1]], x0, args = (params, setpoint, input), t_eval = time, 
                          method = 'Radau')
    
    if not sol.success:
        raise RuntimeError(f"ODE integration failed during MPC: {sol.message}")
